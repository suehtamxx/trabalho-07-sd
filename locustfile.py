...
# ==========================================
# GATILHO AUTOMÁTICO DE WARM-UP
# ==========================================
@events.test_start.add_listener
def on_test_start(environment, **kwargs):
    def reset_stats_after_warmup():
        gevent.sleep(60) # O Locust aguarda exatamente 60 segundos
        print("\n" + "="*50)
        print("WARM-UP DE 1 MINUTO CONCLUÍDO. ZERANDO ESTATÍSTICAS AUTOMATICAMENTE!")
        print("="*50 + "\n")
        environment.runner.stats.reset_all()

    # Roda esse contador em segundo plano junto com o teste
    gevent.spawn(reset_stats_after_warmup)
    
class APIUser(HttpUser):
    # 40% dos acessos (O endpoint com o gargalo)
    @task(4)
    def recurso_lento(self):
        self.client.get("/api/recurso-lento")

    # 30% dos acessos (Consulta indexada por ID)
    @task(3)
    ...
    # 20% dos acessos (Healthcheck)
    @task(2)
    def status(self):
        self.client.get("/api/status")

    # 10% dos acessos (Cadastro de novos dados)
    @task(1)
